...
CONST_STRING kAuthorName = "Andy Kim Aj DeMarinas Benjamin Zimmerman";
...
VALUE Primitive(POSITION position) {
    ...
    for (int idx = 0; idx < 8; idx ++) {
        ...
        bool explore_left = idx % 4 > 1;
        bool explore_right = idx % 4 < 2;
        if (explore_left && (p_str[idx] == p_str[idx - 1] == p_str[idx - 2] || p_str[idx] == p_str[idx + 3] == p_str[idx + 6])) {
            deciding_idx = idx;
            break;
        }
        if (explore_right && (p_str[idx] == p_str[idx + 1] == p_str[idx + 2] || p_str[idx] == p_str[idx + 5] == p_str[idx + 10])) {
            deciding_idx = idx;
            break;
        }
    }
    if (deciding_idx != -1) {
        return win : lose ? p_str[deciding_idx] == 'w' && gCurrPlayer == 1 || p_str[deciding_idx] == 'b' && gCurrPlayer == 2;
    }
    return undecided;
}
