...
CONST_STRING kHelpTieOccursWhen =
    "There are no ties in Jan4x4, but misere has draws!";
...
#define ISMAINDIAG(idx) (idx / 4 == idx % 4 || idx / 4 + idx % 4 == 3)
  ...
  for (int idx = 0; idx < 16; idx++) {
    ...
    for (int cnt = 0; cnt < 8; cnt++) {
      ...
      if (diff == 5 || diff == -5 || diff == 3 || diff == -3) {
        // wraparound
        if (abs(idx % 4 - jdx % 4) > 1)
          continue;

        // game settings
        if (!gAllDiag && !gMainDiag)
          continue;

        if (gMainDiag && (!ISMAINDIAG(idx) || !ISMAINDIAG(jdx)))
          continue;
      }
...
// finds primitive position of 4x4 boards
VALUE standardPrimitive(POSITION position) {
  char board[BOARDSIZE];
  generic_hash_unhash(position, board);

  // checks all diags and vertical loss conditions + horizontal loss conditions
  // for first two rows
  for (int idx = 0; idx < BOARDSIZE / 2; idx++) {
    ...
    if (board[idx] == board[idx + 4] &&
        board[idx + 4] == board[idx + 8]) // down
      return lose;

    if (idx % 4 > 1 && board[idx] == board[idx - 1] &&
        board[idx - 1] == board[idx - 2]) // left
      return lose;

    if (idx % 4 > 1 && board[idx] == board[idx + 3] &&
        board[idx + 3] == board[idx + 6]) // left diagonal
      return lose;

    if (idx % 4 < 2 && board[idx] == board[idx + 1] &&
        board[idx + 1] == board[idx + 2]) // right
      return lose;

    if (idx % 4 < 2 && board[idx] == board[idx + 5] &&
        board[idx + 5] == board[idx + 10]) // right diagonal
      return lose;
  }

  // check bottom 2 rows manually
  if (board[9] != ECHAR && (board[9] == board[10]) &&
      (board[8] == board[9] || board[10] == board[11]))
    return lose;

  if (board[13] != ECHAR && (board[13] == board[14]) &&
...
VALUE Primitive(POSITION position) {
  VALUE result = standardPrimitive(position);
  if (result == lose)
...
void MoveToString(MOVE move, char *moveStringBuffer) {
  snprintf(moveStringBuffer, MAX_MOVE_STRING_LENGTH, "%d %d",
           DECODEMOVEIDX(move) + 1, DECODEMOVEJDX(move) + 1);
...
// standard or misere with
// no, main, or all diagonal moves
int NumberOfOptions() { return 6; }

// returns current variant selected
int getOption() {
  int option = 1;
  if (gAllDiag)
    option = 2;
...
void PositionToAutoGUIString(POSITION position,
  ...
  AutoGUIMakePositionString(generic_hash_turn(position), board,
...
void MoveToAutoGUIString(POSITION position, MOVE move,
  ...
  AutoGUIMakeMoveButtonStringM(DECODEMOVEIDX(move), DECODEMOVEJDX(move), 's',
                               autoguiMoveStringBuffer);
}
